...
export const addNewCategoryToLibrary = (array, library, video) => {
	...
	if (
		itemInMyLibrary &&
		itemInMyLibrary.checked &&
		!videoAlreadyExists(itemInMyLibrary, video)
	) {
		return array.map((item) => {
		...
		return array.map((item) => {
			if (item.id === library.id) {
				return {
					...item,
					videoList: item.videoList.filter((item) => item.id !== video.id),
				};
			}
			return item;
		});
...
export const removeVideoFromACategory = (library, category, videoId) => {
	return library.map((item) => {
		if (item.name === category) {
			return {
				...item,
				videoList: item.videoList.filter((video) => video.id !== videoId),
			};
		}
		return item;
	});
	...
	return videoList.map((video) => {
		if (video.id === videoId) {
			return {
				...
				notes: [
					...video.notes,
					{ id: uuidv4(), note: note.note, time: formatTime(note.time) },
				],
			};
		}
		return video;
